...
// Hide Tooltip Helper
const hideTooltip = () => {
  const tt = document.querySelector(".word-tooltip"); if (tt) tt.remove();
};
...
export async function loadSentencesForLevel(level) {
  ...
  try {
    ...
    const data = await r.json(); sentenceCache[level]=data;
    return data;
  } catch {
...
export const handleDragEnd=_=> draggedItem=null;
...
document.getElementById("level-select").addEventListener("change", async e => {
  currentLevel = e.target.value; await resetQuiz();
});

// Success animation
function animateSuccessMessage() {
  if(window.gsap) gsap.fromTo(
    elements.successMessage,{scale:0,opacity:0},{scale:1,opacity:1,duration:0.6,ease:"bounce.out"}
  );
}

// Init
document.addEventListener("DOMContentLoaded", async () => {
  await generatePuzzles(); displayCurrentPuzzle();
});
